...
@register_criterion("rtl_knowledge_distillation")
class RtlKnowledgeDistillationCriterion(FairseqCriterion):
    def __init__(self, args, task):
        ...
        self.ltr_len=11 # Numbers of ltr(left to right) models. The rest are rtl models
    ...
    def forward(self, model, sample, reduce=True):
        ...
        avg_probs = None
        tgt_len = (sample["target"].cpu().numpy()==2).argmax(axis=1)
        #print(tgt_len)
        for i,teacher_model in enumerate(self.teacher_models):
            ...
            if i>=self.ltr_len:
                ...
                for j in range(len(tgt_len)):
                    probs[j,:int(tgt_len[j])]=probs[j,:int(tgt_len[j])][::-1]
