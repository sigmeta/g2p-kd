...
from fairseq import utils as pytorch_translate_utils


@register_criterion("rerank_knowledge_distillation")
class RerankKnowledgeDistillationCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # Teacher models score for each other, and select the best probability distribution for each sentence
        teacher_probs_list = []
        for teacher_model in self.teacher_models:
            ...
            # all teacher probs
            teacher_probs_list.append(torch.unsqueeze(probs,-1))
        # all teacher probs and lprobs
        teacher_probs = torch.cat(teacher_probs_list,-1)
        teacher_lprobs = torch.log(teacher_probs)
        rerank_loss_list=[]
        # score (calculate loss) for each teacher
        for i in range(teacher_lprobs.size()[-1]):
            rerank_loss_list.append(-torch.sum(teacher_lprobs[:,:,:,i].unsqueeze(-1)*teacher_probs,[1,2,3]).unsqueeze(-1))
        rerank_losses=torch.cat(rerank_loss_list,-1)
        agm=rerank_losses.argmin(1) # choose best
        avg_probs=teacher_probs_list[0]
        #print("avg_probs", avg_probs.size())
        for i in range(avg_probs.size()[0]):
            avg_probs[i]=teacher_probs_list[agm[i]][i]
        avg_probs.squeeze_()
        #print("avg_probs",avg_probs.size())
